<template>
  <div class="apps">
    <div class="screen-c" :id="box">
      ...
      <LoadingVscreen v-if="showLoading" />
      ...
      <img :class="isPlaying ? 'musicIcon musicIcon-active' : 'musicIcon'" v-if="showMusicIcon"
        @click.stop="toggleMusicPlay()" src="../images/pc/music.png" alt="music" />

      <audio id="bg-audio" :src="mp3Url">…</audio>

      <!-- back -->
      <img class="backIcon" v-if="showBackIcon" @click.stop="handlerBack()" src="../images/pc/back.png" alt="music" />
    </div>
  </div>
</template>
<script>
import CarSelect from "@/components/CarSelectPc-vscreen-2k.vue";
import LoadingVscreen from "@/components/Loading-pc-vscreen-2k.vue";
import Gacrender from "../utils/Gacrender.js";
import qs from "qs";
export default {
  components: { CarSelect, LoadingVscreen },
  name: "ViEw",
  data() {
    return {
  ...
  methods: {
    ...
    getUrlParam() {
      ...
      // }
        ...
        this.screenShow = this.params.screenShow.toUpperCase()
        let obj = {
